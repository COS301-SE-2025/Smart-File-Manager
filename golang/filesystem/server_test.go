...
func TestAPI_EndpointsRegistered(t *testing.T) {
	mux := http.NewServeMux()
	// Register all your handlers here
	mux.HandleFunc("/addDirectory", addCompositeHandler)
	mux.HandleFunc("/addTag", addTagHandler)
	mux.HandleFunc("/removeTag", removeTagHandler)
	mux.HandleFunc("/loadTreeData", loadTreeDataHandlerGoOnly)
	mux.HandleFunc("/sortTree", sortTreeHandler)
	mux.HandleFunc("/startUp", startUpHandler)
	mux.HandleFunc("/lock", lockHandler)
	mux.HandleFunc("/unlock", unlockHandler)
	mux.HandleFunc("/deleteFile", deleteFileHandler)
	mux.HandleFunc("/deleteFolder", deleteFolderHandler)
	mux.HandleFunc("/deleteManager", deleteManagerHandler)
	mux.HandleFunc("/search", SearchHandler)
	mux.HandleFunc("/keywordSearch", KeywordSearchHadler)
	mux.HandleFunc("/isKeywordSearchReady", IsKeywordSearchReadyHander)
	mux.HandleFunc("/moveDirectory", moveDirectoryHandler)
	mux.HandleFunc("/findDuplicateFiles", findDuplicateFilesHandler)
	mux.HandleFunc("/bulkAddTag", BulkAddTagHandler)
	mux.HandleFunc("/bulkRemoveTag", BulkRemoveTagHandler)
	mux.HandleFunc("/bulkDeleteFiles", deleteFileHandler)
	mux.HandleFunc("/bulkDeleteFolders", BulkDeleteFolderHandler)
	mux.HandleFunc("/returnType", ReturnTypeHandler)
	mux.HandleFunc("/returnStats", StatHandler)

	server := httptest.NewServer(mux)
	defer server.Close()

	endpoints := []string{
		"/addDirectory", "/addTag", "/removeTag", "/loadTreeData", "/sortTree", "/startUp",
		"/lock", "/unlock", "/deleteFile", "/deleteFolder", "/deleteManager",
		"/search", "/keywordSearch", "/isKeywordSearchReady", "/moveDirectory", "/findDuplicateFiles",
		"/bulkAddTag", "/bulkRemoveTag", "/bulkDeleteFiles", "/bulkDeleteFolders", "/returnType", "/returnStats",
	}
	for _, ep := range endpoints {
		resp, err := http.Get(server.URL + ep)
		if err != nil {
			t.Errorf("Failed to GET %s: %v", ep, err)
			continue
		}
		if resp.StatusCode == http.StatusNotFound {
			t.Errorf("Endpoint %s not registered (404)", ep)
		}
		resp.Body.Close()
	}
}
